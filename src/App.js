...
import { pData1 , pData2 , pData3 , pData4 , pData5 , pData6 , pData7 } from "./assets/arr";

function App() {

// below data will be obtained from an API. Taking static values for the screen development

  const [price, setPrice] = React.useState(pData5[pData5.length - 1].toLocaleString());
  ...
  function showGraph (x){
    console.log(x)
    if(x === 0){setPData(pData1);setPrice(pData1[pData1.length - 1].toLocaleString())}
    if(x === 1){setPData(pData2);setPrice(pData2[pData2.length - 1].toLocaleString())}
    if(x === 2){setPData(pData3);setPrice(pData3[pData3.length - 1].toLocaleString())}
    if(x === 3){setPData(pData4);setPrice(pData4[pData4.length - 1].toLocaleString())}
    if(x === 4){setPData(pData5);setPrice(pData5[pData5.length - 1].toLocaleString())}
    if(x === 5){setPData(pData6);setPrice(pData6[pData6.length - 1].toLocaleString())}
    if(x === 6){setPData(pData7);setPrice(pData7[pData7.length - 1].toLocaleString())}
  }
